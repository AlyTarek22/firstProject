...
<div class="container" style="margin-top: 100px;">
  <h1 style="color: #d65622; font-size: 32px;">Edit Course</h1>
   
  
  <!-- <form action="{{ route('courses.store') }}" method="post">
  @csrf  -->
  <form action="{{ route('courses.update.test', $course->id) }}" method="POST" enctype="multipart/form-data">
      @method('POST')
  <!-- @if(isset($course))
@else
...
<!-- @PUT() -->
  ...
  <div class="row mb-2">
    ...
    <!-- <div class="col-sm-6">
      <label for="">Youtube Vedio Link:</label>
      <br>
      <input type="url" name="youtube_link" id="" required class="w-100 form-control">
  ...
  <div class="row mb-2">
    <div class="col-sm-12">
      ...
      <br>
      <input type="file" name="image" id="" class="w-100 form-control" >
      <a href="{{asset('pictures/'.$course["image"])  }}" target="_blank">{{ old('image', $course->image ?? '') }}</a>
    </div>
  </div>
  ...
  <div class="d-flex flex-column align-items-center">
    ...
    <button id="Save_course_bttn" type="submit">Update Course</button>
